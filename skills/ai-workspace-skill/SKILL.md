...
description: >-
  ...
  process contracts, references, templates, scripts, data, and artifacts
...
Create or review AI workspaces whose operating contract is a skill.

This skill is for the skill-based version of an AI workspace:

- `SKILL.md` is the triggerable workflow controller.
- `process/` holds workspace-level method contracts for durable knowledge.
...
- `data/` holds durable source evidence and distilled knowledge.
- `artifacts/` holds generated reports, decks, and other deliverables.
...
the workspace-local method contract.
...
normalization, ranking, rendering, conversion, or repeatable artifact creation.
...
repositories. Keep repo-specific `data/` and `artifacts/` outside the packaged
...
- identify the domain, user role, recurring workflow, and output artifacts;
...
workspace-local method contract that the skill loads and applies.
...
Create API process files only when the workspace ingests external systems:
...
files, then adapt it to the domain.
...
`SKILL.md` must tell the host when to load the reporting process file and which
...
- schema validation;
- fixture-based artifact validation;
...
- preserve useful `process/` contracts as workspace-local method files;
...
- `SKILL.md` names the process files it depends on;
- referenced files exist;
- process files exist and match the data layout;
...
- process contracts match layer names and object types;
- data and artifact paths match `SKILL.md`;
- report/deck templates and export routes match process contracts;
